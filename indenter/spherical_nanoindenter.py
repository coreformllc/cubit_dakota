...
def build_indenter( geom_params ):
    ...
    cubit.cmd( "nodeset 1 vertex 1" )
    cubit.cmd( "nodeset 1 name 'indenter_tip'")
    
    # cubit.cmd( "create planar surface with plane normal to curve 5 close_to vertex 2" )
    # cubit.cmd( "create vertex on curve 2 crossing surface 2" )
    # cubit.cmd( f"split surface 1 through vertex 2 {cubit.get_last_id( 'vertex' )}" )  
    # cubit.cmd( "delete surface 2" )
    cubit.cmd( "delete vertex all" )
    cubit.cmd( "compress" )
    
    # cubit.cmd( "curve in sideset 1 size 5.0" )  # Far from contact
    # cubit.cmd( "curve in sideset 2 size 5.0" ) # Far from contact
    ...
    generate_mesh_size_function.doIndenter()
...
def build_target( geom_params ):
    ...
    # cubit.cmd( "curve 7 size 5.0 # Far from contact" )
    # cubit.cmd( "curve 8 size 5.0 # Far from contact" )
    ...
    generate_mesh_size_function.doTarget()
    cubit.cmd( "delete mesh" )
